...
public class VideoView extends SurfaceView implements MediaController.MediaPlayerControl {
     ...
     */
    public void setVideoSegments(String[] uris, String cacheDir) {
        mUris = uris;
        mUri = Uri.parse(mUris[0]);
        mCacheDir = cacheDir;
        mSeekWhenPrepared = 0;
        openVideo();
        requestLayout();
    ...
    private void openVideo() {
        ...
        try {
            ...
            if (mUris != null && mUris.length > 1)
                mMediaPlayer.setDataSegments(mUris, mCacheDir);
            else
    ...
    private void attachMediaController() {
        if (mMediaPlayer != null && mMediaController != null) {
            ...
            if (mVideoName == null && mUri != null) {
                List<String> paths = mUri.getPathSegments();
                String name = paths == null || paths.isEmpty() ? "无题" : paths.get(paths.size() - 1);
                mVideoName = name;
            } 
...
//    }
    ...
    public void setVideoSegments(ArrayList<VideoSegment> segments, String cacheDir) {
        String[] uris = new String[segments.size()];
        guessTotal = 0;
        for (int i = 0; i < segments.size(); i++) {
            uris[i] = segments.get(i).url;
            guessTotal += segments.get(i).duration;
        }
        setVideoSegments(uris, cacheDir);
    }

    @Override
    public void changeVideoLayout(int layout, float aspectRatio) {
