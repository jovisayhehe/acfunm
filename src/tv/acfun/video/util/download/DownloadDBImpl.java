...
public class DownloadDBImpl implements DownloadDB {
        ...
        if(mDb != null){
            ...
            try{
                for(query.moveToFirst();!query.isAfterLast();query.moveToNext()){
                    ...
                    // 是否同一个part
                    if(job == null || !job.getEntry().aid.equals(aid) || !job.getEntry().part.sourceId.equals(vid)){
                        job = buildJob(query);
                        all.add(job);
                    }
                    ...
                    if(AcApp.isExternalStorageAvailable()){
                        ...
                        // 实现边下边播的功能 
                            ...
                            if(query.getInt(query.getColumnIndex(COLUMN_CURRENT)) != downloadedSize){
                                ...
                                updateDownload(job.getEntry().part.sourceId,s.num,values);
                            }
                        }
                    }
    ...
    public int remove(DownloadJob job) {
        ...
        String[] args = new String[]{entry.aid,entry.part.sourceId};
    ...
    public void addDownload(DownloadEntry entry) {
        ...
        for(int i =0;i < entry.part.segments.size(); i++){
            ...
            int rowCount = mDb.update(DOWNLOAD_TABLE, values, whereClause, new String[]{entry.aid,entry.part.sourceId,String.valueOf(s.num)});
    ...
    public int updateDownload(DownloadJob job) {
        ...
        for(int i =0;i < entry.part.segments.size(); i++){
            ...
            values.put(COLUMN_NUM, s.num);
            values.put(COLUMN_URL, s.stream);
            rowCount = mDb.update(DOWNLOAD_TABLE, values, whereClause, new String[]{entry.aid,entry.part.sourceId,String.valueOf(s.num)});
        }
        return rowCount;
    }
